...
export default function ScanDetail() {
  ...
  const download = async () => {
    ...
    setDownloading(true);
    try {
      const sourceFile = imageBlob;
      if (!sourceFile) {
        setDownloadError('Original image not available for this scan – cannot regenerate the report.');
        setDownloading(false);
        return;
      }
      const blob = await generateReport(sourceFile as File, 'pdf');
      downloadBlob(blob, `MetroGuard_Inspection_Report_${scan.id.split('-')[0].toUpperCase()}.pdf`);
    } catch (e) {
  ...
  const scanName = `Scan-${scan.id.split('-')[0].toUpperCase()}`;
    ...
    <div className="p-8 max-w-7xl mx-auto pb-20 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${badgeColors[scan.overall_status]}`}>
              {badgeLabels[scan.overall_status]}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
        ...
        <div className="lg:col-span-7 space-y-8">
            ...
            {scan.overall_status === 'insufficient_image_quality' ? (
              ...
              <div className="divide-y divide-slate-100">
                  ...
                  >
                    <div className="sm:w-1/3 shrink-0">
                      ...
                      <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider border ${
                        ...
                        {f.status === 'not_detected' ? 'Review Required' : f.status.replace('_', '-')}
                      </span>
                    </div>
                      ...
                      {f.status === 'not_detected' ? (
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-[11px] font-medium text-slate-500">
                            <span>Rule: <span className="text-blue-600 underline decoration-blue-200 underline-offset-2">{f.rule_ref}</span></span>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                <motion.div 
                  ...
                  className="p-8 bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden"
                >
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <button 
